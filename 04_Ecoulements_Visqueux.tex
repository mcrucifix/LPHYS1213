...
TODO ! ICI: c'est le moment d'attirer l'attention sur les conditions relatives à la symmétrie, et l'absence de termes d'advection. Nous avons donc considérer que la symmétrie du problème se reflète dans la symmétrie des lignes de courant, ce qui génère $\pd{u}{x}=0$ et $\pd{u}{r}=0$  donc annihile tous les termes d'advection. En pratique c'est ce qu'on appelle un écoulement laminaire et developpé. 
...
TODO: expliquer qu'on est à Re $\ll$1 ;  On va outrageusement négliger tous les termes d'advection. Ici c'est clairement une approximation car les lignes de courant sont courbes. 
...
TODO: dire qu'on considère un écoulement tel que $\u=U\ex$ loin de la sphère, la sphère de rayon $a$, et vitesse nulle autour de la sphère. 

L'équation pour un écoulement visqueux stationnaire à nombre de Reynolds très petit est  (TODO: montrer que c'est équivalent à $\lapl\u$ quand $\Div\u=0$). 
...
Ce choix garanti $\Div \u=0$ (TODO: ajouter page avec opérateurs pour coordonnées curvilinéaires). 
...
\begin{equation}
  \left(\pd[2]{}{r} + \frac{\sin\theta}{r^2}\pd{}{\theta}\left(\frac{1}{\sin\theta}\pd{}{\theta}\right)^2 \right) \sfn_z = 0. 
...
\begin{equation}
  D = \int_0^{2\pi}\int{0}^{\pi} t_x a^2 \sin\theta\, \dif \theta\, \dif\varphi = 6\pi \mu U a
