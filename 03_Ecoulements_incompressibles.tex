...
TODO: dire qu'elle exprime la conservation du moment angulaire. Sans moment de force externe, pas de génération de vorticité nette à l'intérieur du fluide. 
...
\centering
    ...
    \raisebox{1em}{
    \parbox{5cm}{\captionof{figure}{Schéma de la cuve et de l'écoulement libre non-turbulent à travers l'orifice.} \label{fig:ecoulement-libre-application}}}
\end{application}
...
\begin{displaymath}
  \ln z = \ln (re^{i(\theta + 2\pi k)}) = \underbrace{ln r}{\vpot} + i\underbrace{\theta + 2\pi k}{\sfn}. 
\end{displaymath}
...
\begin{figure}[h!]
    ...
    \begin{minipage}[t]{0.49\textwidth}
        \centering
        \input{Figures/potential_flow_source.eepic}
    ...
    \begin{minipage}[t]{0.49\textwidth}
        \centering
        \input{Figures/potential_flow_circulation.eepic}
...
\begin{equation}
  \begin{split}
    u + iv = U (1 - \frac{R^2}{z z} &= U(1 - R^2\frac{ {z^\star}^2}{|z|^4})
...
\begin{figure}
\includegraphics{Figures/zero_circulation.pdf}
\label{fig:zero_circulation}
\caption{Fonction de courant (traits pleins) et lignes de potentiel de vitesse (pontillés) pour un fluide parfait rencontrant un obstacle cylindrique ($\vcomp(z) = U ( z + \frac{R^2}{z})$)}
\end{figure}

TODO: expliquer que c'est une transformation conforme. 
...
La suite du calcul, bien que technique, peut être esquissée comme suit. La méthode consiste à exprimer la force résultante d'abord comme un vecteur $(D,L)$, où $D$ représente la \cindex{traînée} (\emph{drag}) et $L$ la \cindex{portance} (\emph{lift}). On calcule le nombre complexe $D - iL$ en voyant que, lors de l'intégrale sur le contour :
\[
...
Pour un écoulement incompressible, irrotationnel, autour d'un obstacle, avec circulation, les valeurs de traînée et de portance sont données par :
\[
